...
def form_tambah_pengajuan(user):
    ...
    # Buat ID unik berdasarkan jam sekarang (biar gak kembar)
    id_unik = "REQ-" + datetime.now().strftime("%H%M%S")
    tanggal_sekarang = datetime.now().strftime("%Y-%m-%d")

    # 1. Siapkan data baris baru dalam bentuk Dictionary
    data_baru = {
        "id": id_unik,
        ...
        "nominal": int(nominal_input), # Pastikan jadi angka
        "status": "Menunggu"
    }
    ...
    # 3. Ubah dictionary data_baru menjadi DataFrame (tabel kecil 1 baris)
    tabel_baru = pd.DataFrame([data_baru])
    
    # 4. Gabungkan tabel lama dengan tabel baru
...
def lihat_status_saya(user):
    ...
    if data_milik_saya.empty:
        ...
        # Tampilkan kolom-kolom penting saja biar gak kepanjangan
